...
public final class ElasticClient implements DocumentStore {
    ...
    public List<Document> get(Set<String> ids, String type, EzSecurityToken userToken) throws TException {
        ...
        return get(idQuery, type, null, null, null, null, 0, MAX_PAGE_SIZE, null, userToken)
    ...
    public List<PercolateQuery> percolate(List<Document> docs) {
        ...
        for (final Document doc : docs) {
            if (doc.getPercolate() != null) {
                ...
                try {
                    ...
                    multiPercolate.add(
                            new PercolateRequestBuilder(client)
                                    .setDocumentType(doc.get_type())
                                    .setIndices(indexName)
                                    .setSize(doc.getPercolate().getMaxMatches())
                                    .setPercolateDoc(new PercolateSourceBuilder.DocBuilder().setDoc(docJson.toString())));
        ...
        for (final MultiPercolateResponse.Item response : responses) {
            ...
            logger.debug("PercolateByIds: Response Time: {} ms, Shards: [{} succ {} fail {} total], match count = {}",
            ...
            if (!response.isFailure()) {
                for (final PercolateResponse.Match match : percolateResponse.getMatches()) {
                    ...
                    percolateQuery.setMatchingDocId(docIds.get(docIndex).toString());
    ...
    @Override
    public List<PercolateQuery> percolateByIds(List<String> ids, String type, int maxMatches) {
        final Stopwatch watch = LoggingUtils.createStopWatch();
        final List<PercolateQuery> results = new ArrayList<>();
        final MultiPercolateRequestBuilder multiPercolate = client.prepareMultiPercolate();
        ...
        for (final MultiPercolateResponse.Item response : responses) {
            ...
            if (!response.isFailure()) {
                for (final PercolateResponse.Match match : percolateResponse.getMatches()) {
                    PercolateQuery percolateQuery = new PercolateQuery();
                    percolateQuery.setId(match.getId().toString());
                    percolateQuery.setMatchingDocId(ids.get(docIndex).toString());
                    results.add(percolateQuery);
                }
            }
            docIndex++;
    ...
    @Override
    public boolean deletePercolator(String id) throws TException {
        final Stopwatch watch = LoggingUtils.createStopWatch();

        DeleteResponse response = client.prepareDelete().setIndex(indexName).setId(id).setType(PERCOLATOR_TYPE).get();
    ...
    @Override
    public Visibility getPercolatorVisibility(String id) throws TException {
        GetResponse response = client.prepareGet().setIndex(indexName).setId(id).setType(PERCOLATOR_TYPE).get();
        if (response.isExists()) {
            return ThriftUtils.deserializeFromBase64(Visibility.class, response.getSource().get(VISIBILITY_FIELD).toString());
        }
        return null;
    }
